...
import {
  Plus, ChevronLeft, ChevronRight, X, Clock, MapPin, Briefcase,
} from 'lucide-react'
...
export default function CalendarPage() {
  ...
  const handleSubmit = () => {
    ...
    setEvents([...events, {
      id: events.length + 1, title: formData.title, type: formData.type, date: formData.date,
      time: formData.time, location: formData.location, case: formData.caseId || '-',
      color: formData.type === 'hearing' ? '#ef4444' : formData.type === 'deadline' ? '#f59e0b' : formData.type === 'consultation' ? '#3b82f6' : '#8b5cf6',
    }])
    setShowForm(false)
    ...
    <AppLayout>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={() => setSelectedEvent(null)}>
          <div className="glass-card w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => setSelectedEvent(null)} className="p-1.5 rounded-lg hover:bg-[var(--accent-light)]"><X className="w-5 h-5 text-[var(--text-muted)]" /></button>
            </div>
      ...
      {showForm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm" onClick={() => setShowForm(false)}>
          <div className="glass-card w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-bold text-[var(--text-primary)]">{t('calendar.newEvent')}</h2>
